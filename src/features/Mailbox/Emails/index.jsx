...
//     <Card key={`email${email.id}`} sx={{marginLeft: 3, marginRight: 3, marginBottom: 1}}>
//       <CardContent>
//       <Grid container
//       direction="row"
...
export default function Emails( {emails, setEmails, selectedEmails, setSelectedEmails} ) {
  ...
  const getData = async () => {
    ...
    const res = await fetch(`https://my-json-server.typicode.com/workinideas/vagafrontendteste/items/${EmailsId}`);
  ...
  return (
    <>
    {emails.length < 1 ? <div>Loading...</div> : <PopulateEmailList /> }
    </>
  )
}